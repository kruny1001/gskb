...
GenesetFields = []
GenesetFieldNames = []
for f in Genesets2._meta.fields:
...
def geneset(request):
    ...
    try:
        ...
        g = Genesets2.objects.get(label=inLabel)
    except Genesets2.DoesNotExist:
        raise Http404
...
from genesets2.forms import SearchForm
...
def gsCommonRestrict(qset, form):
    '''Common code between gsall() and gssearch() for narrowing our dataset.
    Returns a Q with additional constraints'''

    inSpecies=form.cleaned_data['species']
    if inSpecies != 'all':
        qset &= Q(species=inSpecies)

    inSource = form.cleaned_data['sources']
    if inSource != 'all':
        qset &= Q(source=inSource)
...
def gsall(request):
    ...
    # without them.
    # Disabled for now since the person may want to make multiple changes
    #form.fields['sources'].widget.attrs['onclick'] = 'this.form.submit();'

    results = Genesets2.objects.all() # By default, user gets all records

    if form.is_valid():
        qset = gsCommonRestrict(Q(), form)
        if len(qset) > 0:
            results = Genesets2.objects.filter(qset).distinct()
        # If data requested in non-html formats
        encoding = form.cleaned_data['encoding']
        if encoding != 'html':
            return render_non_html(encoding, results)

    return render_to_response('genesets/search2.html', locals())

def gssearch(request):
    '''Geneset search function'''

    import os
    print >> sys.stderr, os.environ['PATH_INFO'], "SCRIPT_NAME: ", os.environ['SCRIPT_NAME']
...
# This is the alternative version that doesn't depend on mysql's full text
    ...
    qset &= gsCommonRestrict(qset, form)
